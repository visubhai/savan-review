...
import { Star, ExternalLink, Check, Sparkles, ArrowRight } from 'lucide-react';
import { KeyboardVisualMockup } from './KeyboardVisualMockup';
  ...
  return (
    <div className="w-full flex flex-col gap-3">
      ...
      <button
        ...
        className={`w-full min-h-[66px] py-4 px-4 sm:px-6 rounded-2xl text-base sm:text-lg font-black flex items-center justify-center gap-2.5 transition-all duration-200 cursor-pointer active:scale-[0.98] shadow-2xl touch-manipulation border ${
          copied
            ...
            : 'bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500 hover:from-orange-400 hover:to-amber-400 text-slate-950 border-orange-400 google-glow-active ring-4 ring-orange-400/40 shadow-orange-500/40'
        }`}
      >
        {copied ? (
          <>
          ...
          <>
            ...
            <span className="tracking-tight text-center">{t.copyAndOpenBtn}</span>
            <ExternalLink className="w-5 h-5 ml-0.5 opacity-90 stroke-[2.5] shrink-0" />
          </>
        )}
      </button>

      {/* VISUAL PHONE KEYBOARD MOCKUP (DUMMY-PROOF) */}
      <KeyboardVisualMockup lang={lang} reviewPreview={textToCopy} />
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-3xl p-5 text-white shadow-2xl text-center space-y-4">
            ...
            <div className="bg-slate-800/90 rounded-2xl p-3.5 text-left space-y-2.5 text-xs border border-slate-700 font-semibold">
              <div className="flex items-center gap-2.5">
                <span className="w-6 h-6 rounded-full bg-amber-500 text-slate-950 font-black flex items-center justify-center text-xs shrink-0">1</span>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <span>કીબોર્ડ પરથી લખાણ <strong>Paste / ટચ</strong> કરો</span>
              </div>
              <div className="flex items-center gap-2.5">
                <span className="w-6 h-6 rounded-full bg-blue-500 text-white font-black flex items-center justify-center text-xs shrink-0">3</span>
